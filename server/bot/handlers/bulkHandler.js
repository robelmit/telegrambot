...
exports.handleBulk = handleBulk;
exports.handleBulkDocument = handleBulkDocument;
exports.handleBulkDoneCallback = handleBulkDoneCallback;
exports.handleBulkCancelCallback = handleBulkCancelCallback;
exports.handleBulkDone = handleBulkDone;
exports.handleBulkCancel = handleBulkCancel;
exports.isInBulkMode = isInBulkMode;
...
async function handleBulk(ctx) {
    ...
    if (!telegramId) {
        await ctx.reply((0, locales_1.t)(lang, 'error_generic'));
        return;
    }
...
📋 Results will be combined into PDFs (${FILES_PER_PDF} IDs per PDF)`;
    const keyboard = telegraf_1.Markup.inlineKeyboard([
        [telegraf_1.Markup.button.callback('✅ Process Files', 'bulk_done')],
        [telegraf_1.Markup.button.callback('❌ Cancel', 'bulk_cancel')]
    ]);
 ...
 */
async function handleBulkDocument(ctx) {
    const telegramId = ctx.from?.id;
    ...
    if (user) {
        ...
        if (session.files.length >= maxAffordable) {
            ...
            await ctx.reply(`💰 You can only afford ${maxAffordable} files with your current balance.\n` +
                `Tap Process to continue or top up for more.`, keyboard);
            return;
        }
    }
    // Validate file type
    ...
    try {
        ...
        const tempPath = path_1.default.join(tempDir, `bulk_${telegramId}_${(0, uuid_1.v4)()}.pdf`);
        await promises_1.default.writeFile(tempPath, pdfBuffer);
        // Add to session
        session.files.push({
            path: tempPath,
            name: fileName
        ...
        const keyboard = telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.callback(`✅ Process ${session.files.length} Files`, 'bulk_done')],
            [telegraf_1.Markup.button.callback('❌ Cancel', 'bulk_cancel')]
        ]);
        await ctx.reply(`✅ File ${session.files.length}/${MAX_BULK_FILES} received: ${fileName}\n\n` +
            `📄 Will generate: ${pdfCount} combined PDF${pdfCount > 1 ? 's' : ''}\n` +
            (remaining > 0
                ? `📤 Send more files or tap Process when ready.`
                : `📦 Maximum files reached. Tap Process to continue.`), keyboard);
    }
    catch (error) {
        logger_1.default.error('Bulk file upload error:', error);
 ...
 */
async function handleBulkDoneCallback(ctx) {
 ...
 */
async function handleBulkCancelCallback(ctx) {
    await ctx.answerCbQuery();
 ...
 */
async function handleBulkDone(ctx) {
    const lang = ctx.session.language || 'en';
    const telegramId = ctx.from?.id;
    if (!telegramId) {
        await ctx.reply((0, locales_1.t)(lang, 'error_generic'));
    ...
    // Disable bulk mode
    ctx.session.awaitingBulkPdf = false;
    const processingMsg = await ctx.reply(`⏳ Processing ${session.files.length} files...\n` +
    ...
    try {
        ...
        const bulkJobId = `bulk_${(0, uuid_1.v4)()}`;
        const debitSuccess = await walletService.debit(user._id.toString(), totalCost, bulkJobId);
        if (!debitSuccess) {
            await ctx.telegram.editMessageText(ctx.chat.id, processingMsg.message_id, undefined, '❌ Insufficient balance. Please top up and try again.');
            await cleanupBulkSession(telegramId);
            return;
        }
        ...
        await ctx.telegram.editMessageText(ctx.chat.id, processingMsg.message_id, undefined, `✅ ${session.files.length} files queued for processing!\n\n` +
        ...
        logger_1.default.error('Bulk processing error:', error);
        // Refund on error
        try {
            await walletService.refund(user._id.toString(), totalCost, `bulk_refund_${Date.now()}`);
        }
        catch (refundError) {
            logger_1.default.error('Refund failed:', refundError);
        }
        await ctx.telegram.editMessageText(ctx.chat.id, processingMsg.message_id, undefined, '❌ Failed to process files. Your balance has been refunded.');
        await cleanupBulkSession(telegramId);
    }
}
/**
 ...
 */
async function handleBulkCancel(ctx) {
    const telegramId = ctx.from?.id;
    if (!telegramId) {
        return;
    }
 ...
 */
async function cleanupBulkSession(telegramId) {
    const session = bulkSessions.get(telegramId);
    if (session) {
 ...
 */
function isInBulkMode(ctx) {
    return ctx.session.awaitingBulkPdf === true;
}
//# sourceMappingURL=bulkHandler.js.map