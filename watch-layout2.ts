/**
 * Watch cardLayout2.json (Template 3) and re-render on save
 * Uses real data from fayda.pdf
 * Run with: npx ts-node watch-layout2.ts
 */
...
async function loadPdfData(): Promise<ParsedData> {
  ...
  console.log(`Checking for PDF at: ${path.resolve(PDF_PATH)} - exists: ${pdfExists}`);
...
async function render() {
  ...
  fs.writeFileSync(path.join(OUTPUT_DIR, 'front2_color.png'), frontCanvas.toBuffer('image/png'));
  ...
  fs.writeFileSync(path.join(OUTPUT_DIR, 'back2_color.png'), backCanvas.toBuffer('image/png'));

  console.log(`[${new Date().toLocaleTimeString()}] ✓ Rendered front2_color.png & back2_color.png (Template 3 with real PDF data)`);
}

// Initial render
console.log('Watching cardLayout2.json (Template 3) for changes...');
